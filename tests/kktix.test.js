...
(async () => {
  ...
  assert.equal(list.children[0].children[0].children[0].textContent, '<b>節目</b>（2 個票種）', 'render group names as text');
  assert.equal(list.children[1].children[1].textContent, '一般票');
  assert.equal(list.children[0].children[0].children[0].href, 'https://kktix.com/events/a/registrations/new');
  assert.equal(list.children[1].children[2].children[0].textContent, '×');
  const drag = { dataTransfer: { setData() {} }, preventDefault() {} };
  list.children[1].children[0].children[0].dragstart(drag);
  await list.children[4].drop(drag);
  assert.deepEqual(data['KktixTicketScope:a'], ['ticket_1', 'ticket_2'], 'cross-group drop ignored');
  await list.children[2].drop(drag);
  assert.deepEqual(Array.from(data['KktixTicketScope:a']), ['ticket_2', 'ticket_1']);
  assert.deepEqual(data['KktixTicketScope:b'], ['ticket_1']);
  list.children[2].children[0].children[0].dragstart(drag);
  await list.children[1].drop(drag);
  assert.deepEqual(Array.from(data['KktixTicketScope:a']), ['ticket_1', 'ticket_2']);
  await list.children[1].children[2].children[0].click();
  ...
  await list.children[0].children[0].children[1].click();
  assert.equal(data['KktixTicketScope:a'].length, 0, 'clear one group');
  assert.deepEqual(data['KktixTicketScope:b'], ['ticket_1'], 'other group remains');
